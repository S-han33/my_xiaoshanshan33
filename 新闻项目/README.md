...
> **修改目的**：新增 AI 关键词过滤，只显示 AI 相关新闻

**修改内容：**
1. `news.py` — 新增 `is_ai_related()` 函数，根据中英文 AI 关键词（AI、人工智能、大模型、GPT、OpenAI、Agent、机器学习、深度学习、自动驾驶等）过滤新闻标题和摘要
2. `news.py` — `job()` 中去重排序后增加 AI 关键词过滤步骤，80条原始新闻过滤后约剩30-40条
3. `news.py` — 无 AI 新闻时跳过更新，保留旧页面

**效果：** 之前会显示"沪深两市成交额突破1.5万亿元"等财经新闻，现在只显示 AI 相关内容
